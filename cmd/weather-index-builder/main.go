...
type Config struct {
	...
	IndexPath  string            `json:"index_path" usage:"specify the location of the index" default:"weather.bleve"`
...
func main() {
	...
	app := &cli.App{
		...
		Action: func(ctx *cli.Context) error {
			clock := clocks.Extract(ctx.Context)

			geocodingAPI := geocoding.NewClient()
			weatherAPI := weather.NewClient()

			geocodeResp, err := geocodingAPI.SearchByAddress(ctx.Context, &cfg.Address)
			if err != nil {
				return err
			}
			...
			point, err := weatherAPI.GetPoint(ctx.Context, coordinates.Y, coordinates.X)
			if err != nil {
				return err
			}

			gridpoints, err := weatherAPI.GetGridpoint(ctx.Context, point.GridID, point.GridX, point.GridY)
			if err != nil {
				return err
			}
